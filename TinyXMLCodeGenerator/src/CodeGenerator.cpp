...
bool CodeGenerator::parseDirectories() {
		...
		++it)
	{
		...
		try {
			...
			lastCount = (unsigned int)m_keywordlist.size();
		}
...
void CodeGenerator::generateKeywordList() {
	...
	if (m_keywordlist.empty()) {
		IBK::IBK_Message("No keywords. Skipping generation of keyword list.\n", IBK::MSG_PROGRESS, FUNC_ID);
		return;
	}
	...
	std::string fileprefix = "cg_" + m_prefix;
	...
	CPP_HEADER.replace(CPP_HEADER.find("${LICENSEHEADER}"),16, m_licenseHeader);
	...
	HEADER.replace(HEADER.find("${PREFIX}"),9, m_prefix);
	HEADER.replace(HEADER.find("${PREFIX}"),9, m_prefix);
	HEADER.replace(HEADER.find("${LICENSEHEADER}"),16, m_licenseHeader);


	// replace two occurrances of namespace placeholder
	QT_HEADER.replace(QT_HEADER.find("${NAMESPACE}"),12, m_namespace);
	QT_HEADER.replace(QT_HEADER.find("${PREFIX}"),9, m_prefix);
	QT_HEADER.replace(QT_HEADER.find("${PREFIX}"),9, fileprefix);
	QT_HEADER.replace(QT_HEADER.find("${PREFIX}"),9, fileprefix);
	QT_HEADER.replace(QT_HEADER.find("${PREFIX}"),9, fileprefix);
	QT_HEADER.replace(QT_HEADER.find("${PREFIX}"),9, fileprefix);
	QT_HEADER.replace(QT_HEADER.find("${LICENSEHEADER}"),16, m_licenseHeader);
	QT_CPP_HEADER.replace(QT_CPP_HEADER.find("${NAMESPACE}"),12, m_namespace);
	QT_CPP_HEADER.replace(QT_CPP_HEADER.find("${PREFIX}"),9, fileprefix);
	QT_CPP_HEADER.replace(QT_CPP_HEADER.find("${LICENSEHEADER}"),16, m_licenseHeader);

	// generate target dir for keyword list
	IBK::Path parentDir;
	if (m_ncgOutputDir.isAbsolute())
		parentDir = m_ncgOutputDir;
	else
		parentDir = m_inputDirectories[0] / m_ncgOutputDir;

	// write qt header
	if ( m_isQtOutputRequired ) {
		IBK::Path outputPath(parentDir / "qt");
		if (!outputPath.exists())
			throw IBK::Exception(IBK::FormatString("Output directory '%1' doesn't exist.").arg(outputPath), FUNC_ID);
		IBK::Path fnameQt = outputPath / ("cg_" + m_prefix +"_KeywordListQt.h");
		if (!fnameQt.exists())
	...
	try {
		// generate code
		IBK::Path fnameCPP = parentDir / ("cg_" + m_prefix +"_KeywordList.cpp");
		generateKeywordlistCode(fnameCPP);
		// generate qt code
		if ( m_isQtOutputRequired ){
			IBK::Path fnameQt = parentDir / ("qt/cg_" + m_prefix +"_KeywordListQt.cpp");
	...
	for (const ClassInfo & ci : m_classInfo) {
		try {
			...
			if (ci.m_xmlInfo.empty() && !ci.m_requireComparisonFunction) {
				if (!ci.m_className.empty())
					IBK::IBK_Message(IBK::FormatString("File with declaration for class '%1' does not contain // XML: lines nor requires comparison operator. Skipped.\n").arg(ci.m_className), IBK::MSG_PROGRESS);
				else
					IBK::IBK_Message(IBK::FormatString("File does not contain // XML: lines nor requires comparison operator. Skipped.\n"), IBK::MSG_PROGRESS);
				continue;
			}
			...
			IBK::Path targetFile = parentDir / "cg_" + ci.m_sourceHeaderFile.filename().withoutExtension().str() + ".cpp";
			...
			if (targetFile.exists()) {
				...
				if (targetFile.fileTime() > ci.m_sourceHeaderFile.fileTime()) {
					IBK::IBK_Message(IBK::FormatString("Target file '%1' is newer than '%2', skipped.\n")
									 .arg(targetFile).arg(ci.m_sourceHeaderFile), IBK::MSG_PROGRESS, FUNC_ID, IBK::VL_STANDARD);
					continue;
				}
			...
			bool haveWriteOnlyIfDifferentChecks = false;
			for (const ClassInfo::XMLInfo & xmlInfo : ci.m_xmlInfo) {
				if (xmlInfo.element) continue;
				if (xmlInfo.writeOnlyIfDifferent)
					haveWriteOnlyIfDifferentChecks = true;
			}

			std::string attribs;
			// add code for generating default-constructed object, unless already done for attributes
			if (haveWriteOnlyIfDifferentChecks) {
				attribs += "	"+ci.m_className+" DEFAULT;\n";
			}
			...
			for (const ClassInfo::XMLInfo & xmlInfo : ci.m_xmlInfo) {
				...
				if (xmlInfo.writeOnlyIfDifferent) {
					attribs += 	"	if (m_"+attribName +" != DEFAULT.m_"+attribName+")\n	";
				}
					...
					xmlInfo.typeStr == "bool")
				{
					...
					attribs +=
							"	if (m_" + attribName + ".isValid())\n"
							"		e->SetAttribute(\""+attribName+"\", m_" + attribName + ".toString(\"yyyy-MM-dd hh:mm:ss\").toStdString());\n";
				}
				else if (xmlInfo.typeStr == "QDate") {
					attribs +=
							"	if (m_" + attribName + ".isValid())\n"
							"		e->SetAttribute(\""+attribName+"\", m_" + attribName + ".toString(\"yyyy-MM-dd\").toStdString());\n";
				}
				else if (xmlInfo.typeStr == "QTime") {
					attribs +=
							"	if (m_" + attribName + ".isValid())\n"
							"		e->SetAttribute(\""+attribName+"\", m_" + attribName + ".toString(\"hh:mm:ss\").toStdString());\n";
				}
			...
			bool haveWriteOnlyIfDifferentChecksForElements = false;
			for (const ClassInfo::XMLInfo & xmlInfo : ci.m_xmlInfo) {
				if (!xmlInfo.element) continue;
			...
			// add code for generating default-constructed object, unless already done for attributes
			if (haveWriteOnlyIfDifferentChecksForElements && !haveWriteOnlyIfDifferentChecks) {
				elements += "	"+ci.m_className+" DEFAULT;\n";
			}
			...
			for (const ClassInfo::XMLInfo & xmlInfo : ci.m_xmlInfo) {
				...
				//   - std::vector<double|int|unsigned int|std::string>
				...
				if (xmlInfo.writeOnlyIfDifferent) {
					elements += "	if (m_"+varName +" != DEFAULT.m_"+varName+")\n	";
				}
					...
					xmlInfo.typeStr == "bool")
				{
				...
				else if (xmlInfo.typeStr == "char") {
					// TiXmlElement::appendSingleAttributeElement(e, "parity", nullptr, std::string(), std::string(m_parity);
					elements +=
							"	TiXmlElement::appendSingleAttributeElement(e, \""+tagName+"\", nullptr, std::string(), std::string(1, m_"+varName+"));\n";
				}
				else if (xmlInfo.typeStr == "std::string") {
					elements +=
							"	if (!m_" + varName + ".empty())\n"
					...
					elements +=
							...
							"		TiXmlElement::appendSingleAttributeElement(e, \""+tagName+"\", nullptr, std::string(), m_"+varName+".toString(\"yyyy-MM-dd hh:mm:ss\").toStdString());\n";
				}
				else if (xmlInfo.typeStr == "QDate") {
					elements +=
							"	if (m_" + varName + ".isValid())\n"
							"		TiXmlElement::appendSingleAttributeElement(e, \""+tagName+"\", nullptr, std::string(), m_"+varName+".toString(\"yyyy-MM-dd\").toStdString());\n";
				}
				else if (xmlInfo.typeStr == "QTime") {
					elements +=
							"	if (m_" + varName + ".isValid())\n"
							"		TiXmlElement::appendSingleAttributeElement(e, \""+tagName+"\", nullptr, std::string(), m_"+varName+".toString(\"hh:mm:ss\").toStdString());\n";
				}
				else if (xmlInfo.typeStr == "IBK::MultiLanguageString") {
					elements +=
							"	if (!m_" + varName + ".empty())\n"
					...
					elements +=
							"	if (!m_" + varName + ".empty())\n"
							"		CODEGEN::writeLinearSplineElement(e, \""+tagName+"\", m_"+varName+", \"-\", \"-\");\n";
				}
...
//							"			IBK_ASSERT(\""+tagName+"\" == m_"+varName+"[i].m_name);\n"
					...
					if (childType == "double" || childType == "int" || childType == "unsigned int"  || childType == "std::string") {
						includes.insert("CodeGen_Utilities.h");
						// we generate the parent element and afterwards the loop
						elements += "	CODEGEN::writeVector(e, \""+tagName+"\", m_"+varName+");\n";
					}
					else if (childType == "IBKMK::Vector3D") {
						includes.insert("CodeGen_Utilities.h");
						// we generate the parent element and afterwards the loop
						elements += "	CODEGEN::writeVector3D(e, \""+tagName+"\", m_"+varName+");\n";
					}
					else if (childType == "QuotedString") {
						includes.insert("CodeGen_Utilities.h");
						// we generate the parent element and afterwards the loop
						elements += "	CODEGEN::writeVectorQuotedString(e, \""+tagName+"\", m_"+varName+");\n";
					}
					else {
						// we generate the parent element and afterwards the loop
						elements += "\n"
					...
					if (childType == "double" || childType == "int" || childType == "unsigned int") {
						includes.insert("CodeGen_Utilities.h");
						// we generate the parent element and afterwards the loop
						elements += "	CODEGEN::writePoint2D(e, \""+tagName+"\", m_"+varName+");\n";
					}
					else
			...
			if (haveAttribs) {
				...
				for (const ClassInfo::XMLInfo & xmlInfo : ci.m_xmlInfo) {
					...
					{
						attribs +=
							"				m_"+attribName+" = CODEGEN::readPODAttributeValue<"+xmlInfo.typeStr+">(element, attrib);\n";
						includes.insert("CodeGen_Utilities.h");
						elseStr = "else ";
					}
					else if (xmlInfo.typeStr == "IDType") {
						attribs +=
							"				m_"+attribName+" = (IDType)CODEGEN::readPODAttributeValue<unsigned int>(element, attrib);\n";
						includes.insert("CodeGen_Utilities.h");
						elseStr = "else ";
					}
					else if (xmlInfo.typeStr == "IBK::Path") {
						...
						attribs +=
							"				m_"+attribName+" = QDateTime::fromString(QString::fromStdString(attrib->ValueStr()), \"yyyy-MM-dd hh:mm:ss\");\n";
						elseStr = "else ";
					}
					else if (xmlInfo.typeStr == "QDate") {
						attribs +=
							"				m_"+attribName+" = QDate::fromString(QString::fromStdString(attrib->ValueStr()), \"yyyy-MM-dd\");\n";
						elseStr = "else ";
					}
					else if (xmlInfo.typeStr == "QTime") {
						attribs +=
							"				m_"+attribName+" = QTime::fromString(QString::fromStdString(attrib->ValueStr()), \"hh:mm:ss\");\n";
						elseStr = "else ";
					}
					else if (xmlInfo.typeStr == "QColor") {
						attribs +=
				...
				for (const ClassInfo::XMLInfo & xmlInfo : ci.m_xmlInfo) {
						...
						xmlInfo.typeStr == "bool")
					{
						...
						elements +=
							...
							"				m_"+varName+" = CODEGEN::readPODElement<"+xmlInfo.typeStr+">(c, cName);\n";
						...
						if (pos1 != std::string::npos) {
							...
							"						m_"+varName+"[i] = (IDType)CODEGEN::readPODElement<unsigned int>(c, cName);\n"
							"						found = true;\n"
							"						break;\n"
							"					}\n"
							"				}\n";
							...
							elements +=
								"			"+elseStr+"if (cName == \""+tagName+"\")\n"
								"				m_"+varName+" = (IDType)CODEGEN::readPODElement<unsigned int>(c, cName);\n";
						}
						handledVariables.insert(varName);
					}
					else if (xmlInfo.typeStr == "std::string") {
						...
						elements +=
							"			"+elseStr+"if (cName == \""+tagName+"\")\n"
							"				m_"+varName+" = QDateTime::fromString(QString::fromStdString(c->GetText()),\"yyyy-MM-dd hh:mm:ss\");\n";
						handledVariables.insert(varName);
					}
					else if (xmlInfo.typeStr == "QDate") {
						elements +=
							"			"+elseStr+"if (cName == \""+tagName+"\")\n"
							"				m_"+varName+" = QDate::fromString(QString::fromStdString(c->GetText()),\"yyyy-MM-dd\");\n";
						handledVariables.insert(varName);
					}
					else if (xmlInfo.typeStr == "QTime") {
						elements +=
							"			"+elseStr+"if (cName == \""+tagName+"\")\n"
							"				m_"+varName+" = QTime::fromString(QString::fromStdString(c->GetText()),\"hh:mm:ss\");\n";
						handledVariables.insert(varName);
					}
					else if (xmlInfo.typeStr == "IBK::MultiLanguageString") {
						elements +=
						...
						elements +=
							"			"+elseStr+"if (cName == \""+tagName+"\")\n"
							"				m_"+varName+" = CODEGEN::readUnitElement(c, cName);\n";
						handledVariables.insert(varName);
					}
					else if (xmlInfo.typeStr == "IBK::Time") {
						includes.insert("CodeGen_Utilities.h");
						elements +=
							"			"+elseStr+"if (cName == \""+tagName+"\")\n"
							"				m_"+varName+" = CODEGEN::readTimeElement(c, cName);\n";
						handledVariables.insert(varName);
					}
					else if (xmlInfo.typeStr == "IBK::Path") {
						elements +=
					...
					// for the group types, we generate the code only once and switch the parameter in an internal loop
						...
						elements +=
							...
							"				IBK::LinearSpline p;\n"
							"				std::string name;\n"
							"				CODEGEN::readLinearSplineElement(c, p, name, nullptr, nullptr);\n";
						...
						elements +=
							...
							"				CODEGEN::LinearSplineParameter p;\n"
							"				p.readXML(c);\n";
					...
					// *** end groups (static array) handling
						...
						if (childType == "double" || childType == "int" || childType == "unsigned int" || childType == "std::string") {
							...
							elements +=
									"			"+elseStr+"if (cName == \""+tagName+"\")\n"
									"				CODEGEN::readVector(c, \""+tagName+"\", m_"+varName+");\n";
						}
						else if (childType == "IBKMK::Vector3D") {
							// we generate the parent element and afterwards the loop
							elements +=
									"			"+elseStr+"if (cName == \""+tagName+"\")\n"
									"				CODEGEN::readVector3D(c, \""+tagName+"\", m_"+varName+");\n";
						}
						else if (childType == "QuotedString") {
							// we generate the parent element and afterwards the loop
							elements +=
									"			"+elseStr+"if (cName == \""+tagName+"\")\n"
									"				CODEGEN::readVectorQuotedString(c, \""+tagName+"\", m_"+varName+");\n";
						}
						else {
							...
							if (childTagName.find("CODEGEN::") == 0)
						...
						if (childType == "double" || childType == "int" || childType == "unsigned int") {
							...
							elements +=
									"			"+elseStr+"if (cName == \""+tagName+"\")\n"
									"				CODEGEN::readPoint2D(c, \""+tagName+"\", m_"+varName+");\n";
						}
			...
			fileHeader = IBK::replace_string(fileHeader, "${NAMESPACE}", m_namespace);
			fileHeader = IBK::replace_string(fileHeader, "${LICENSEHEADER}", m_licenseHeader);
