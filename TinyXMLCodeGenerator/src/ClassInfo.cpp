...
bool ClassInfo::parse(const IBK::Path & headerFilePath) {
	...
	while (std::getline(in, line)) {
		try {
			if (line.find("CODEGEN_READWRITE_PRIVATE") != std::string::npos)
			{
				m_requirePrivateReadWrite = true;
				continue;
			}
			if (line.find("CODEGEN_COMP") != std::string::npos)
			{
...
//					std::cout << enumName << ":" << enumIdx << std::endl;
			...
			std::size_t posXML = line.find("XML:");
			pos = line.find("// XML:");
			if (posXML != std::string::npos && pos == std::string::npos)
				std::cout << "Found 'XML:' in line:\n  " << line << "\nbut expected '// XML:...'. Maybe tab instead of space used?" << std::endl;
...
//				std::cout << typeStr << " : " << varName << " : " << xmlSpec << std::endl;
				...
				for (unsigned int i=1; i<tokens.size(); ++i) {
					...
					else if (tokens[i] == "write-if-different") {
						xmlInfo.writeOnlyIfDifferent = true;
					}
					else {
