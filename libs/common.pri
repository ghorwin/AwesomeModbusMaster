...
# This is the central configuration file for all libraries/applications.
...
equals(TEMPLATE,app) {
	...
	CONFIG(debug, debug|release) {
		QMAKE_LIBDIR += ../libs/lib/debug ../lib/debug
		LIBS += -L../lib/debug -L../libs/lib/debug
	}
	else {
		QMAKE_LIBDIR += ../libs/lib/release ../lib/release
		LIBS += -L../lib/release -L../libs/lib/release
	}
...
#	message(Setting up ordinary library support.)
	...
	CONFIG(debug, debug|release) {
		...
		#       we need to set the linker path such, that it finds other libraries in <repo-root>/libs/lib
		contains( OPTIONS, top_level_libs ) {
			LIBS += -L../libs/lib/debug -L../lib/debug
			QMAKE_LIBDIR += ../libs/lib/debug ../lib/debug
			windows {
		...
		DESTDIR = ../lib/release
		contains( OPTIONS, top_level_libs ) {
			LIBS += -L../libs/lib/release -L../lib/release
			QMAKE_LIBDIR += ../libs/lib/release ../lib/release
			windows {
				# place windows DLLs into executable target directory
				DLLDESTDIR = $$BINARYDESTDIR
