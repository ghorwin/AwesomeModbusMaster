...
/*! Extracts and returns the function code, does not modify the query */
void modbus_extract_package_data(modbus_t * ctx, const uint8_t * req, int req_length,
								 int *functionCode, uint16_t * address)
{
	int offset;

	offset = ctx->backend->header_length;
	*functionCode = req[offset];
